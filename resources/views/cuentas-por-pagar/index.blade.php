    ...
    <div class="row">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center mb-4">
                ...
                <div>
                    <a href="{{ route('cuentas-por-pagar.reporte-antiguedad') }}" class="btn btn-outline-info me-2">
                        <i class="fas fa-chart-bar"></i>
                        Reporte de Antigüedad
                    </a>
                    <a href="{{ route('cuentas-por-pagar.exportar-excel') }}" class="btn btn-outline-success">
            ...
            <div class="card border-0 shadow-sm bg-danger text-white">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <h6 class="card-title text-white-50 mb-0">Total por Pagar</h6>
                            <h3 class="mb-0">${{ number_format($estadisticas['total_por_pagar'], 2) }}</h3>
                        </div>
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-3">
                            ...
                            <input type="text" class="form-control" id="filtroProveedor" placeholder="Buscar proveedor...">
                        </div>
                        <div class="col-md-2">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-hover align-middle" id="tablaCuentasPorPagar">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge bg-secondary">{{ $cuenta['categoria'] }}</span>
                                    </td>
                                    ...
                                    <td>
                                        <div class="btn-group btn-group-sm">
                                            <a href="{{ route('cuentas-por-pagar.show', $cuenta['id']) }}" 
                                            ...
                                            <button class="btn btn-outline-warning" 
                                                    onclick="mostrarModalProgramar({{ $cuenta['id'] }})" title="Programar pago">
            ...
            <form id="formProgramarPago" method="POST">
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-warning">
                        <i class="fas fa-calendar-check"></i> Programar Pago
                    </button>
                </div>
            </form>
        </div>
...
function mostrarModalPago(cuentaId) {
    ...
    form.action = `/cuentas-por-pagar/${cuentaId}/pago`;
    
    // Limpiar formulario
    form.reset();
    
    // Establecer fecha actual
    document.querySelector('#modalRegistrarPago input[name="fecha_pago"]').value = new Date().toISOString().split('T')[0];
...
function mostrarModalProgramar(cuentaId) {
    const form = document.getElementById('formProgramarPago');
    form.action = `/cuentas-por-pagar/${cuentaId}/programar`;
    ...
    document.querySelector('#modalProgramarPago input[name="fecha_programada"]').min = tomorrow.toISOString().split('T')[0];
    
    // Mostrar modal
    const modal = new bootstrap.Modal(document.getElementById('modalProgramarPago'));
    modal.show();
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const tabla = document.getElementById('tablaCuentasPorPagar');
    
    function filtrarTabla() {
        const filas = tabla.querySelectorAll('tbody tr');
        const proveedor = filtroProveedor.value.toLowerCase();
        const estado = filtroEstado.value;
        const categoria = filtroCategoria.value;
        
        filas.forEach(fila => {
            const textoProveedor = fila.children[1].textContent.toLowerCase();
            const textoCategoria = fila.children[2].textContent.trim();
            const textoEstado = fila.children[8].textContent.trim();
            
            const coincideProveedor = proveedor === '' || textoProveedor.includes(proveedor);
            ...
            fila.style.display = coincideProveedor && coincideEstado && coincideCategoria ? '' : 'none';
        });
    }
    
    filtroProveedor.addEventListener('input', filtrarTabla);
